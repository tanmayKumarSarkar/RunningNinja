...
public class NewHighScoreScrpt : MonoBehaviour {
	...
	void Start () {
		...
		AudioSource.PlayClipAtPoint(GameOverSnd2, transform.position);
	}
	...
	void OnGUI() {
		...
		if (GUI.Button (
			new Rect ((36 * adjustedWidth / 80), (7.5f * nativeHeight / 20), buttonWidth, buttonHeight), "NEW HIGH SCORE !!")) {
		...
		if (GUI.Button (
			...
			AudioSource.PlayClipAtPoint(BtnSnd, transform.position);
			Application.LoadLevel("GameScenes");
		}
	}
